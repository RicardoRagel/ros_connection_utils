# Source ROS and your workspace setup scripts and connect to another machine roscore
# USAGE: $ source init_ros_client.sh (optional) <roscore_ip> (optional) <workspace_name>

if [ "$1" == "-h" ]
then
    echo "USAGE: $ source init_ros_client.sh (optional) <roscore_ip> (optional) <workspace_name>"
    echo "If you do not want any workspace use: --no-ws"
    return
fi

ROSCORE_IP="192.168.0.122"
CATKIN_WS="catkin_ws"
USING_CATKIN_WS=1
...
echo "Initializing ROS connection to another roscore"
echo "----------------------------------------------"
echo "Assuming your IP is $MY_IP and the roscore-machine IP is $ROSCORE_IP"
